...
export class Log {
    ...
    public static getMsg(level: (typeof Levels)[number], data: unknown[] | unknown): string {
        const msg: string[] = [];
    ...
    public static critical(...data: unknown[]): void {
    ...
    public info(...data: unknown[]) {
        Log.info((this ?? { name: '[unknown]' }).name, ...data);
    }

    public warn(...data: unknown[]) {
        Log.warn((this ?? { name: '[unknown]' }).name, ...data);
    }

    public error(...data: unknown[]) {
        Log.error((this ?? { name: '[unknown]' }).name, ...data);
    }

    public critical(...data: unknown[]) {
        Log.critical((this ?? { name: '[unknown]' }).name, ...data);
    }

    public debug(...data: unknown[]) {
        Log.debug((this ?? { name: '[unknown]' }).name, ...data);
    }
}
...
process.on('uncaughtException', (e) => {
    Log.critical(false, '=== Unhandled Exception ===');
    Log.critical(false, e);
    Log.createCrash();
    process.exit(1);
});

process.on('unhandledRejection', (e) => {
    Log.critical(false, '=== Unhandled Rejection ===');
    Log.critical(false, e);
    Log.createCrash();
    process.exit(1);
});
